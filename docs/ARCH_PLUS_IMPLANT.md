...
This document defines the ORA_CORE_RAG implant for ARCH+ v3, the advanced M10 Persona Genesis Engine.

## Scope

The implant builds an ArchiPersona activation packet from a JSON payload. It is route-gated by `ClientRouteGate` and returns a structured profile with source status per field.
...
ORA_CORE_RAG may index the public module spec, schema, tests and docs. Tenant profile payloads must remain in tenant-scoped storage behind a valid GLK route.
